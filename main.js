...
let screenShake=0,screenFlash=0,regionFade=0,regionTitleTime=0,hitStop=0,currentQuiz=null,quizSceneAnimation=0,saveSyncTimer=null;
...
function quizQuestions(stage){const themes=data.themes.slice(stage*3,stage*3+3),pool=themes.flatMap(t=>t.words),known=pool.filter(w=>state.learned.includes(w.id)),source=(known.length>=10?known:pool).sort(()=>Math.random()-.5).slice(0,10);return source.map((w,i)=>{const reverse=i%2===1,correct=reverse?w.es:w.ru,wrong=data.words.filter(x=>x.id!==w.id).sort(()=>Math.random()-.5).slice(0,3).map(x=>reverse?x.es:x.ru);return {prompt:reverse?`¿Cómo se dice «${w.ru}» en español?`:`¿Qué significa «${w.es}»?`,ru:reverse?'Как это сказать по-испански?':'Что означает это слово?',correct,answers:[correct,...wrong].sort(()=>Math.random()-.5),icon:w.icon}})}
function openStageQuiz(stage,onPass){playFx('quiz');modalOpen=true;currentQuiz={stage,onPass,questions:quizQuestions(stage),index:0,score:0,locked:false};$('#modal-backdrop').classList.remove('hidden');$('#modal-kicker').textContent=`PRUEBA DE LA ETAPA ${stage+1} · ИСПЫТАНИЕ`;$('#modal-title').textContent=['La Prueba del Búho','El Reto de la Torre','La Puerta de Olvido'][stage];renderQuiz()}
function renderQuiz(){const q=currentQuiz.questions[currentQuiz.index],c=$('#modal-content');cancelAnimationFrame(quizSceneAnimation);c.innerHTML=`<div class="quiz-scene upgraded"><canvas id="quiz-battle-canvas" width="760" height="180"></canvas><div class="quiz-scene-title"><span>ETAPA ${currentQuiz.stage+1}</span><b>⚔ PRUEBA DEL HÉROE ⚔</b><small>Испытание героя</small></div><div class="quiz-word-orb">${q.icon}</div></div><div class="quiz-progress long">${currentQuiz.questions.map((_,i)=>`<i class="${i<currentQuiz.index?'done':i===currentQuiz.index?'active':''}"></i>`).join('')}</div><section class="quiz-card"><small>PREGUNTA ${currentQuiz.index+1} / ${currentQuiz.questions.length}</small><h3>${q.prompt}</h3><p>${q.ru}</p><div class="quiz-answers">${q.answers.map(a=>`<button data-quiz-answer="${escapeHtml(a)}">${escapeHtml(a)}</button>`).join('')}</div><div id="quiz-feedback"></div></section>`;$$('[data-quiz-answer]').forEach(b=>b.onclick=()=>answerQuiz(b,b.dataset.quizAnswer));animateQuizBattle(currentQuiz.stage)}
function animateQuizBattle(stage){const canvas=$('#quiz-battle-canvas');if(!canvas)return;const qctx=canvas.getContext('2d'),start=performance.now(),colors=['#62385e','#315f4b','#554a63'];function hero(x,y,t){qctx.save();qctx.translate(x,y+Math.sin(t*3)*2);qctx.fillStyle='#12132466';qctx.beginPath();qctx.ellipse(0,20,19,7,0,0,Math.PI*2);qctx.fill();qctx.fillStyle='#4a3028';qctx.fillRect(-11,11,9,12);qctx.fillRect(3,11,9,12);qctx.fillStyle=state.color||'#52d6ff';qctx.fillRect(-13,-10,26,23);qctx.fillStyle='#513126';qctx.fillRect(-14,5,28,5);qctx.fillStyle='#f0bd91';qctx.fillRect(-10,-28,20,17);qctx.fillStyle='#4e3034';qctx.fillRect(-12,-31,24,8);qctx.fillRect(-12,-25,5,11);qctx.fillStyle='#25243a';qctx.fillRect(3,-21,3,3);qctx.fillStyle='#3b6f8a';qctx.fillRect(-19,-7,7,19);qctx.strokeStyle='#f8f0c9';qctx.lineWidth=5;qctx.beginPath();qctx.moveTo(15,-4);qctx.lineTo(39,-24+Math.sin(t*2)*3);qctx.stroke();qctx.fillStyle='#e7bb4c';qctx.fillRect(11,-8,8,5);qctx.restore()}
 function enemy(x,y,i,t){const r=18+i*2,bob=Math.sin(t*2.4+i)*4;qctx.save();qctx.translate(x,y+bob);qctx.fillStyle='#12102055';qctx.beginPath();qctx.ellipse(2,r*.65,r,7,0,0,Math.PI*2);qctx.fill();qctx.fillStyle=colors[stage%colors.length];qctx.beginPath();qctx.moveTo(-r,r*.55);qctx.quadraticCurveTo(-r,-r*.45,0,-r);qctx.quadraticCurveTo(r,-r*.45,r,r*.55);qctx.lineTo(r*.4,r*.35);qctx.lineTo(0,r*.7);qctx.lineTo(-r*.45,r*.35);qctx.fill();if(i===0){qctx.beginPath();qctx.moveTo(-11,-12);qctx.lineTo(-17,-27);qctx.lineTo(-4,-16);qctx.moveTo(11,-12);qctx.lineTo(17,-27);qctx.lineTo(4,-16);qctx.fill()}if(i===2){qctx.strokeStyle='#c795e8';qctx.lineWidth=3;qctx.beginPath();qctx.arc(0,0,r+6,0,Math.PI*2);qctx.stroke()}qctx.fillStyle='#fff2c7';qctx.fillRect(-9,-8,6,5);qctx.fillRect(4,-8,6,5);qctx.fillStyle='#302039';qctx.fillRect(-7,-7,2,3);qctx.fillRect(6,-7,2,3);qctx.restore()}
 function frame(now){if(!canvas.isConnected)return;const t=(now-start)/1000,w=canvas.width,h=canvas.height;qctx.clearRect(0,0,w,h);const sky=qctx.createLinearGradient(0,0,0,h);sky.addColorStop(0,['#403967','#31554e','#4a3c60'][stage]);sky.addColorStop(1,['#8aae68','#718d69','#746683'][stage]);qctx.fillStyle=sky;qctx.fillRect(0,0,w,h);qctx.fillStyle='#ffffff0a';for(let x=0;x<w;x+=32)for(let y=105;y<h;y+=24)qctx.fillRect(x+(y%48),y,16,2);qctx.fillStyle='#1b183044';qctx.beginPath();qctx.moveTo(0,112);for(let x=0;x<=w;x+=70)qctx.lineTo(x,75+(x*7%45));qctx.lineTo(w,130);qctx.lineTo(0,130);qctx.fill();for(let i=0;i<14;i++){qctx.globalAlpha=.35+.3*Math.sin(t*2+i);qctx.fillStyle='#ffe989';qctx.fillRect((i*83+t*16)%w,35+(i*37)%105,2,2)}qctx.globalAlpha=1;hero(145,126,t);enemy(550,127,0,t);enemy(625,132,1,t);enemy(705,128,2,t);qctx.strokeStyle='#ffe38455';qctx.lineWidth=2;qctx.setLineDash([5,8]);qctx.beginPath();qctx.arc(380,92,29+Math.sin(t*3)*3,0,Math.PI*2);qctx.stroke();qctx.setLineDash([]);quizSceneAnimation=requestAnimationFrame(frame)}quizSceneAnimation=requestAnimationFrame(frame)}
...
function finishQuiz(){const pass=currentQuiz.score>=8,c=$('#modal-content'),q=currentQuiz;c.innerHTML=`<div class="quiz-result ${pass?'pass':'fail'}"><div class="result-hero">${pass?'🏆':'📚'}</div><h2>${pass?'¡Prueba superada!':'Casi, joven héroe'}</h2><p>${pass?`Has acertado ${q.score} de ${q.questions.length}. El camino hacia la nueva tierra está abierto.`:`Has acertado ${q.score} de ${q.questions.length}. Necesitas 8 respuestas correctas.`}</p><p>${pass?'Испытание пройдено. Новый путь открыт.':'Попробуй ещё раз. Нужно дать 8 правильных ответов.'}</p><button id="quiz-finish" class="primary">${pass?'CONTINUAR LA AVENTURA':'REPETIR LA PRUEBA'}</button></div>`;$('#quiz-finish').onclick=()=>{if(pass){if(!state.testsPassed.includes(q.stage))state.testsPassed.push(q.stage);state.coins+=20;save();closeModal();q.onPass?.()}else{currentQuiz.questions=quizQuestions(q.stage);currentQuiz.index=0;currentQuiz.score=0;renderQuiz()}}}
...
function drawHouse(x,y,wall,roof){
 ...
 // sombra y base de piedra
 ctx.fillStyle='#11151a55';ctx.beginPath();ctx.ellipse(x+72,y+86,82,18,0,0,Math.PI*2);ctx.fill();ctx.fillStyle='#5a4a43';ctx.fillRect(x-2,y+69,140,17);for(let i=0;i<9;i++){ctx.fillStyle=i%2?'#75655a':'#67574f';ctx.fillRect(x+i*16,y+70+(i%2)*7,14,7)}
 // pared de yeso con vigas medievales
 ctx.fillStyle=wall;ctx.fillRect(x+3,y+4,132,69);ctx.fillStyle='#ffffff12';ctx.fillRect(x+8,y+8,5,56);ctx.fillStyle='#4c352d';ctx.fillRect(x+2,y+2,7,70);ctx.fillRect(x+129,y+2,7,70);ctx.fillRect(x+3,y+34,132,6);ctx.strokeStyle='#4c352d';ctx.lineWidth=5;ctx.beginPath();ctx.moveTo(x+7,y+5);ctx.lineTo(x+42,y+36);ctx.moveTo(x+131,y+5);ctx.lineTo(x+96,y+37);ctx.stroke();
 // tejado con alero, sombra y tejas individuales
 ctx.fillStyle='#2c2026aa';ctx.beginPath();ctx.moveTo(x-18,y+9);ctx.lineTo(x+68,y-58);ctx.lineTo(x+154,y+9);ctx.closePath();ctx.fill();ctx.fillStyle=roof;ctx.beginPath();ctx.moveTo(x-13,y+3);ctx.lineTo(x+68,y-52);ctx.lineTo(x+149,y+3);ctx.closePath();ctx.fill();ctx.strokeStyle='#ffffff20';ctx.lineWidth=2;for(let row=0;row<5;row++){const yy=y-4-row*9,half=69-row*13;ctx.beginPath();ctx.moveTo(x+68-half,yy);ctx.lineTo(x+68+half,yy);ctx.stroke()}ctx.strokeStyle='#5d3732aa';for(let i=-4;i<=4;i++){ctx.beginPath();ctx.moveTo(x+68,y-49);ctx.lineTo(x+68+i*16,y+2);ctx.stroke()}ctx.fillStyle='#e8b86655';ctx.beginPath();ctx.moveTo(x-8,y);ctx.lineTo(x+68,y-47);ctx.lineTo(x+144,y);ctx.strokeStyle='#f4d58a66';ctx.lineWidth=3;ctx.stroke();
 // chimenea humeante
 ctx.fillStyle='#5a4037';ctx.fillRect(x+106,y-45,18,32);ctx.fillStyle='#7d5b4b';ctx.fillRect(x+102,y-48,26,6);for(let i=0;i<3;i++){ctx.globalAlpha=.18-i*.04;pxCircle(x+116+Math.sin(t+i)*5,y-58-i*12,7+i*2,'#d8d2c4')}ctx.globalAlpha=1;
 // puerta tallada y herrajes
 ctx.fillStyle='#2b2230';ctx.fillRect(x+52,y+36,35,48);ctx.fillStyle='#835437';ctx.fillRect(x+57,y+39,25,45);ctx.strokeStyle='#bc8150';ctx.lineWidth=2;ctx.strokeRect(x+60,y+43,19,16);ctx.strokeRect(x+60,y+63,19,17);ctx.fillStyle='#ffd56a';pxCircle(x+76,y+61,2,'#ffd56a');
 drawWindow(x+15,y+45);drawWindow(x+101,y+45);
 // jardineras y letrero central
 ctx.fillStyle='#75452f';ctx.fillRect(x+11,y+65,32,7);ctx.fillRect(x+96,y+65,32,7);for(let i=0;i<6;i++){pxCircle(x+15+i*5,y+63,3,i%2?'#f8cc64':'#eb7996');pxCircle(x+100+i*5,y+63,3,i%2?'#84d279':'#ad91e8')}ctx.fillStyle='#f1d18a';ctx.fillRect(x+42,y+9,52,10);ctx.fillStyle='#6b4632';ctx.fillRect(x+46,y+12,44,2);ctx.restore()
}
function drawWindow(x,y){ctx.save();ctx.shadowColor='#8fe4e3';ctx.shadowBlur=8;ctx.fillStyle='#3d3048';ctx.fillRect(x-4,y-4,29,27);ctx.shadowBlur=0;const glass=ctx.createLinearGradient(x,y,x+21,y+19);glass.addColorStop(0,'#d8ffff');glass.addColorStop(.35,'#8fe4e3');glass.addColorStop(1,'#4b8fa5');ctx.fillStyle=glass;ctx.fillRect(x,y,21,19);ctx.fillStyle='#fff8cc88';ctx.fillRect(x+3,y+2,4,8);ctx.fillStyle='#493a55';ctx.fillRect(x+9,y,3,19);ctx.fillRect(x,y+8,21,3);ctx.fillStyle='#76513a';ctx.fillRect(x-6,y+20,33,5);ctx.fillStyle='#d8b15a';ctx.fillRect(x-2,y+21,25,2);ctx.restore()}
function drawPond(x,y,t){
 ctx.save();
 // ribera irregular en tres capas
 ctx.fillStyle='#263e35aa';ctx.beginPath();ctx.ellipse(x+3,y+8,105,60,-.03,0,Math.PI*2);ctx.fill();ctx.fillStyle='#8b8a62';ctx.beginPath();ctx.ellipse(x,y+2,100,55,0,0,Math.PI*2);ctx.fill();ctx.fillStyle='#50775d';ctx.beginPath();ctx.ellipse(x,y-1,94,49,0,0,Math.PI*2);ctx.fill();
 const water=ctx.createRadialGradient(x-28,y-20,5,x,y,98);water.addColorStop(0,'#72d5cf');water.addColorStop(.45,'#3aa0ad');water.addColorStop(1,'#226b82');ctx.fillStyle=water;ctx.beginPath();ctx.ellipse(x,y-4,88,43,0,0,Math.PI*2);ctx.fill();
 // reflejo del cielo y ondas móviles
 ctx.globalAlpha=.2;ctx.fillStyle='#d8fff4';ctx.beginPath();ctx.ellipse(x-25,y-20,42,10,-.12,0,Math.PI*2);ctx.fill();ctx.globalAlpha=1;ctx.lineCap='round';for(let i=0;i<5;i++){const yy=y-28+i*13,off=Math.sin(t*1.7+i)*11,w=24+i*8;ctx.strokeStyle=i%2?'#b4f2de77':'#1e718066';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(x-w+off,yy);ctx.quadraticCurveTo(x+off,yy+5,x+w+off,yy);ctx.stroke()}
 // nenúfares, flores, piedras y juncos
 for(const [ox,oy,sz] of [[-35,5,10],[45,-13,8],[18,20,7]]){ctx.fillStyle='#39794d';ctx.beginPath();ctx.arc(x+ox,y+oy,sz,.25,Math.PI*2-.25);ctx.lineTo(x+ox,y+oy);ctx.fill()}pxCircle(x-36,y+2,4,'#f1d470');pxCircle(x+45,y-16,3,'#f4a9d1');for(let i=0;i<6;i++){const a=i*.9+.2;pxCircle(x+Math.cos(a)*96,y+Math.sin(a)*50,4+i%3,'#928f78')}for(let i=0;i<7;i++){const rx=x-86+i*7;ctx.strokeStyle='#426b42';ctx.lineWidth=2;ctx.beginPath();ctx.moveTo(rx,y+22);ctx.quadraticCurveTo(rx-3,y+4,rx+Math.sin(t+i)*2,y-8-i%3*3);ctx.stroke();ctx.fillStyle='#765b35';ctx.fillRect(rx-2,y-11-i%3*3,4,9)}
 ctx.restore()
}
...
function drawCastle(x,y){
 ctx.save();
 // gran sombra y muralla central con degradado de piedra
 ctx.fillStyle='#17142166';ctx.beginPath();ctx.ellipse(x+130,y+170,160,24,0,0,Math.PI*2);ctx.fill();const stone=ctx.createLinearGradient(x,y,x,y+170);stone.addColorStop(0,'#858095');stone.addColorStop(1,'#514b63');ctx.fillStyle=stone;ctx.fillRect(x,y+24,260,145);
 // sillares alternados
 ctx.strokeStyle='#39354455';ctx.lineWidth=1;for(let yy=32;yy<164;yy+=18){for(let xx=(yy/18%2)*15;xx<260;xx+=30)ctx.strokeRect(x+xx,y+yy,28,16)}
 // torres laterales salientes
 for(const tx of [x-24,x+214]){ctx.fillStyle='#625c71';ctx.fillRect(tx,y-3,70,174);ctx.fillStyle='#777184';for(let yy=17;yy<165;yy+=22)for(let xx=5;xx<66;xx+=24)ctx.fillRect(tx+xx,y+yy,20,17);ctx.fillStyle='#3b3649';ctx.fillRect(tx+25,y+58,18,35);ctx.fillStyle='#82c9d3';ctx.fillRect(tx+29,y+62,10,27);ctx.fillStyle='#292535';ctx.fillRect(tx+28,y+75,12,3);for(let i=0;i<4;i++){ctx.fillStyle=i%2?'#6e687b':'#827b8d';ctx.fillRect(tx-2+i*23,y-19,19,25)}}
 // almenas centrales y camino de ronda
 ctx.fillStyle='#6d677b';ctx.fillRect(x+42,y+16,172,20);for(let i=0;i<7;i++)ctx.fillRect(x+43+i*28,y+2,17,23);ctx.fillStyle='#423d50';ctx.fillRect(x+42,y+34,172,7);
 // puerta ojival, madera y rastrillo
 ctx.fillStyle='#252131';ctx.beginPath();ctx.arc(x+130,y+132,38,Math.PI,0);ctx.fill();ctx.fillRect(x+92,y+132,76,38);ctx.fillStyle='#5e3f34';ctx.beginPath();ctx.arc(x+130,y+134,31,Math.PI,0);ctx.fill();ctx.fillRect(x+99,y+133,62,37);ctx.strokeStyle='#b28a53';ctx.lineWidth=3;for(let i=0;i<5;i++){ctx.beginPath();ctx.moveTo(x+105+i*13,y+112);ctx.lineTo(x+105+i*13,y+169);ctx.stroke()}ctx.beginPath();ctx.moveTo(x+99,y+137);ctx.lineTo(x+161,y+137);ctx.stroke();
 // escudo, banderas y ventanas iluminadas
 ctx.fillStyle='#b33d55';ctx.beginPath();ctx.moveTo(x+118,y+52);ctx.lineTo(x+142,y+52);ctx.lineTo(x+139,y+78);ctx.lineTo(x+130,y+86);ctx.lineTo(x+121,y+78);ctx.fill();ctx.fillStyle='#efd167';ctx.font='bold 15px serif';ctx.textAlign='center';ctx.fillText('Ñ',x+130,y+72);for(const bx of [x+5,x+249]){ctx.fillStyle='#4b3340';ctx.fillRect(bx,y-44,5,46);ctx.fillStyle='#b83e57';ctx.beginPath();ctx.moveTo(bx+5,y-42);ctx.lineTo(bx+42,y-31);ctx.lineTo(bx+5,y-18);ctx.fill();ctx.fillStyle='#f2d15e';ctx.fillRect(bx+10,y-35,4,9)}for(const wx of [65,180]){ctx.fillStyle='#252435';ctx.fillRect(x+wx,y+96,16,30);ctx.fillStyle='#8bd7db';ctx.fillRect(x+wx+4,y+100,8,22)}ctx.textAlign='left';ctx.restore()
}
...
function drawTower(x,y,color){
 ctx.save();ctx.fillStyle='#17182166';ctx.beginPath();ctx.ellipse(x+68,y+140,80,17,0,0,Math.PI*2);ctx.fill();const wall=ctx.createLinearGradient(x,y,x+130,y+135);wall.addColorStop(0,color);wall.addColorStop(1,'#4b4858');ctx.fillStyle=wall;ctx.fillRect(x,y,130,137);ctx.strokeStyle='#302e3c55';for(let yy=7;yy<132;yy+=18)for(let xx=(yy%36?0:13);xx<130;xx+=27)ctx.strokeRect(x+xx,y+yy,25,16);
 // almenas, torrecillas y tejado de pizarra
 ctx.fillStyle='#686574';ctx.fillRect(x-5,y-9,140,18);for(let i=0;i<6;i++)ctx.fillRect(x-6+i*28,y-24,20,23);ctx.fillStyle='#383347';ctx.beginPath();ctx.moveTo(x-12,y-24);ctx.lineTo(x+65,y-71);ctx.lineTo(x+142,y-24);ctx.fill();ctx.strokeStyle='#91829d';ctx.lineWidth=2;for(let i=-3;i<=3;i++){ctx.beginPath();ctx.moveTo(x+65,y-67);ctx.lineTo(x+65+i*22,y-25);ctx.stroke()}
 // puerta, ventanas, antorchas decorativas y estandarte
 ctx.fillStyle='#292536';ctx.beginPath();ctx.arc(x+65,y+104,22,Math.PI,0);ctx.fill();ctx.fillRect(x+43,y+103,44,34);ctx.fillStyle='#714936';ctx.fillRect(x+49,y+105,32,32);for(const wx of [20,96]){ctx.fillStyle='#272536';ctx.fillRect(x+wx,y+31,16,31);ctx.fillStyle='#88d2dc';ctx.fillRect(x+wx+4,y+36,8,20)}ctx.fillStyle='#b33d56';ctx.fillRect(x+61,y-67,4,45);ctx.beginPath();ctx.moveTo(x+65,y-63);ctx.lineTo(x+101,y-52);ctx.lineTo(x+65,y-40);ctx.fill();ctx.fillStyle='#efd266';ctx.fillRect(x+73,y-57,4,9);ctx.restore()
}
...
function closeModal(){modalOpen=false;cancelAnimationFrame(quizSceneAnimation);$('#modal-backdrop').classList.add('hidden')}
...
function playFx(name){if(!sound)return;const fx={
 ...
 quiz:()=>{noise(.18,.018);[196,262,330,392,523].forEach((f,i)=>tone(f,.42,i<2?'sawtooth':'triangle',.018,i*.09));tone(784,.55,'sine',.012,.38)},
