...
const freshState=name=>({name:name||'Héroe',color:selectedColor,region:0,x:330,y:350,hp:100,maxHp:100,mana:60,maxMana:60,coins:8,xp:0,level:1,learned:[],inventory:[{id:'potion',qty:1}],rewardedDialogues:[],kills:0,shadowKills:0,bosses:[],completed:[],quest:'meetOwl',questKills:0,sword:1,armor:0,playtime:0,started:Date.now()});
...
function startGame(saved=false){state=saved?load():freshState($('#hero-name').value.trim());if(!state)return;state.mana=state.mana??60;state.rewardedDialogues??=[];
