...
class Enemy{constructor(x,y,opt={}){Object.assign(this,{x,y,type:'enemy',r:15,hp:45,maxHp:45,speed:45,damageVal:10,color:'#62385e',name:'Sombra de Tinta',weak:'hola',boss:false,dead:false,hit:0,cool:0,talkCd:rand(1,3)},opt)}
 update(dt){if(this.dead)return;this.hit=Math.max(0,this.hit-dt);this.cool-=dt;this.talkCd-=dt;let d=dist(this,player);if(d<185&&this.talkCd<=0){enemySay(this,this.boss?{es:'¡Nadie abre mi puerta!',ru:'Никто не откроет мои ворота!'}:ENEMY_PHRASES[Math.floor(Math.random()*ENEMY_PHRASES.length)]);this.talkCd=rand(4.5,7.5)}if(d<260&&d>this.r+player.r){this.x+=(player.x-this.x)/d*this.speed*dt;this.y+=(player.y-this.y)/d*this.speed*dt}if(d<this.r+player.r+4&&this.cool<=0){player.damage(this.damageVal);this.cool=1.1}if(this.boss&&Math.random()<dt*.8){let a=Math.atan2(player.y-this.y,player.x-this.x);projectiles.push({x:this.x,y:this.y,vx:Math.cos(a)*130,vy:Math.sin(a)*130,r:6,life:3,enemy:true,color:'#8c65a8',damage:8})}}
...
function enemySay(enemy,line,final=false){const text=typeof line==='string'?line:line.es,ru=typeof line==='string'?'':line.ru;combatBubbles.push({x:enemy.x,y:enemy.y-enemy.r-14,text,ru,life:final?2.4:1.8,final})}
