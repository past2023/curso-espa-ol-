...
const ENEMY_PHRASES=['¡Alto, héroe!','¡Mi magia es fuerte!','No tienes la palabra.','¡Este bosque es mío!','¡Fuera de mi camino!'];
const ENEMY_FINAL=['La luz es buena...','Hoy ganas tú...','Adiós, héroe...','Tu palabra es fuerte...'];
...
let screenShake=0,screenFlash=0,regionFade=0,hitStop=0,currentQuiz=null,saveSyncTimer=null;
...
function save(show=false){if(!state)return;state.x=player.x;state.y=player.y;localStorage.setItem(SAVE_KEY,JSON.stringify(state));updateRanking();syncServer();if(show)toast('Partida guardada local y en línea · Игра сохранена','💾')}
...
class Enemy{constructor(x,y,opt={}){Object.assign(this,{x,y,type:'enemy',r:15,hp:45,maxHp:45,speed:45,damageVal:10,color:'#62385e',name:'Sombra de Tinta',weak:'hola',boss:false,dead:false,hit:0,cool:0,talkCd:rand(1,3)},opt)}
 update(dt){if(this.dead)return;this.hit=Math.max(0,this.hit-dt);this.cool-=dt;this.talkCd-=dt;let d=dist(this,player);if(d<185&&this.talkCd<=0){enemySay(this,this.boss?'¡Nadie abre mi puerta!':ENEMY_PHRASES[Math.floor(Math.random()*ENEMY_PHRASES.length)]);this.talkCd=rand(4.5,7.5)}if(d<260&&d>this.r+player.r){this.x+=(player.x-this.x)/d*this.speed*dt;this.y+=(player.y-this.y)/d*this.speed*dt}if(d<this.r+player.r+4&&this.cool<=0){player.damage(this.damageVal);this.cool=1.1}if(this.boss&&Math.random()<dt*.8){let a=Math.atan2(player.y-this.y,player.x-this.x);projectiles.push({x:this.x,y:this.y,vx:Math.cos(a)*130,vy:Math.sin(a)*130,r:6,life:3,enemy:true,color:'#8c65a8',damage:8})}}
 damage(n,dir,magic=''){if(this.dead)return;if(magic&&magic===this.weak)n*=2;this.hp-=n;this.hit=.14;screenShake=this.boss?8:4;screenFlash=.09;hitStop=.035;floatingTexts.push({x:this.x,y:this.y-this.r,text:`-${Math.round(n)}`,color:magic?'#73efff':'#fff0a0',life:.75});playFx(magic?'magicHit':'hit');if(Math.random()<.38)enemySay(this,magic?'¡Esa palabra quema!':'¡Ay, mi armadura!');this.x+=(dir?.x||0)*12;this.y+=(dir?.y||0)*12;burst(this.x,this.y,magic?'#6deaff':'#ffbf64',7);if(this.hp<=0)this.die()}
 die(){this.dead=true;enemySay(this,this.boss?'Olvido está cerca...':ENEMY_FINAL[Math.floor(Math.random()*ENEMY_FINAL.length)],true);playFx(this.boss?'bossDown':'coin');screenShake=this.boss?14:5;if(this.boss)screenFlash=.3;let gain=this.boss?30:Math.floor(rand(3,8));spawnCoins(this.x,this.y,gain);state.kills++;state.xp+=this.boss?80:15;burst(this.x,this.y,'#ffd34f',Math.min(16,gain),'spark');if(!this.boss){
...
function spawnRegion(){
 ...
 if(r===2){npc(180,370,'family','Pablo','P','#4b82b4');npc(785,360,'ana','Ana la Valiente','A','#c9637e');foe(470,190);foe(710,170);foe(720,420);foe(520,360,{r:33,hp:220,maxHp:220,speed:32,damageVal:16,color:'#484157',name:'Guardián Gris',weak:'luz',boss:true})}
 ...
 if(r===4){npc(165,310,'ranger','Luna','L','#477c64');npc(760,180,'druid','Roble','R','#5b7041');foe(360,180,{color:'#315f4b',name:'Lobo de Tinta',speed:62});foe(570,450,{color:'#3e674b',name:'Lobo de Tinta',speed:62});foe(820,420,{color:'#465c3d',name:'Espíritu Salvaje',weak:'luz'});foe(610,210,{r:30,hp:190,maxHp:190,color:'#244c3c',name:'Rey Bestia',weak:'fuego',boss:true})}
...
function answerQuiz(button,answer){if(currentQuiz.locked)return;currentQuiz.locked=true;const q=currentQuiz.questions[currentQuiz.index],ok=answer===q.correct;button.classList.add(ok?'correct':'wrong');if(!ok)$$('[data-quiz-answer]').find(b=>b.dataset.quizAnswer===q.correct)?.classList.add('correct');$('#quiz-feedback').innerHTML=ok?'✨ ¡Muy bien! · Правильно!':`📖 La respuesta es <b>${q.correct}</b>`;playFx(ok?'learn':'hurt');if(ok){currentQuiz.score++;burst(player.x,player.y,'#ffe36a',14,'spark')}setTimeout(()=>{currentQuiz.index++;currentQuiz.locked=false;if(currentQuiz.index<currentQuiz.questions.length)renderQuiz();else finishQuiz()},900)}
function finishQuiz(){const pass=currentQuiz.score>=3,c=$('#modal-content'),q=currentQuiz;c.innerHTML=`<div class="quiz-result ${pass?'pass':'fail'}"><div class="result-hero">${pass?'🏆':'📚'}</div><h2>${pass?'¡Prueba superada!':'Casi, joven héroe'}</h2><p>${pass?`Has acertado ${q.score} de 4. La nueva tierra está abierta.`:`Has acertado ${q.score} de 4. Necesitas 3 respuestas correctas.`}</p><p>${pass?'Испытание пройдено. Новый путь открыт.':'Попробуй ещё раз. Нужно 3 правильных ответа.'}</p><button id="quiz-finish" class="primary">${pass?'CONTINUAR LA AVENTURA':'REPETIR LA PRUEBA'}</button></div>`;$('#quiz-finish').onclick=()=>{if(pass){if(!state.testsPassed.includes(q.stage))state.testsPassed.push(q.stage);state.coins+=20;save();closeModal();q.onPass?.()}else{currentQuiz.questions=quizQuestions(q.stage);currentQuiz.index=0;currentQuiz.score=0;renderQuiz()}}}
...
function drawWorld(t){
 ...
 if(state.region===3){
  ...
  drawHouse(75,135,'#c26a42','#f0c05c');drawHouse(725,120,'#c98b42','#e86f45');drawMarket(410,130,'#b74445');
 ...
 if(state.region===6){
  ...
  drawTower(455,95,'#766d78');drawWell(375,435);drawMarket(650,430,'#557ca0');drawSign(180,315,'CIUDAD');
 }
 if(state.region===7){
 ...
 if(state.region===8){
  ...
  drawHedges();drawRuins(420,250);drawTower(780,105,'#574b70');[[260,485],[590,155],[845,460]].forEach(p=>drawMagicCrystal(...p));drawSign(145,310,'DERECHA');
 }
 ...
 afterImages.forEach(drawAfterImage);actors.forEach(a=>a.draw());coinDrops.forEach(drawCoinDrop);projectiles.forEach(drawProjectile);particles.forEach(drawParticle);floatingTexts.forEach(drawFloatingText);combatBubbles.forEach(drawCombatBubble);
 ...
 if(state.region>0)drawExit(12,330,'‹');if(state.region<data.regions.length-1)drawExit(948,330,'›');
}
...
function enemySay(enemy,text,final=false){combatBubbles.push({x:enemy.x,y:enemy.y-enemy.r-14,text,life:final?2.2:1.5,final})}
function drawCombatBubble(b){ctx.save();ctx.globalAlpha=clamp(b.life*2,0,1);ctx.font='bold 11px sans-serif';const w=Math.min(210,Math.max(72,ctx.measureText(b.text).width+20)),x=clamp(b.x-w/2,8,W-w-8),y=b.y-35;ctx.fillStyle=b.final?'#fff3c8':'#f8f5e8';ctx.strokeStyle=b.final?'#d39c3c':'#312942';ctx.lineWidth=3;ctx.beginPath();ctx.roundRect(x,y,w,28,8);ctx.fill();ctx.stroke();ctx.fillStyle=b.final?'#7a3b2a':'#282239';ctx.textAlign='center';ctx.fillText(b.text,x+w/2,y+18);ctx.beginPath();ctx.moveTo(b.x-5,y+28);ctx.lineTo(b.x,y+36);ctx.lineTo(b.x+6,y+28);ctx.fill();ctx.restore()}
...
function updateObjective(){let text=state.quest==='meetOwl'?'Habla con Don Diccionario':state.quest==='killShadows'?`Derrota Sombras (${state.questKills}/3)`:state.quest==='boss'?'Viaja a la Aldea y derrota al Guardián':(data.regions[state.region]?.quest||'Explora Verbalia');$('#objective-text').textContent=text}
...
else loadServerSave().then(remote=>{if(remote){localStorage.setItem(SAVE_KEY,JSON.stringify(remote));$('#continue-btn').classList.remove('hidden');$('#continue-btn').textContent=`CONTINUAR COMO ${remote.name.toUpperCase()}`;toast('Partida recuperada del servidor','☁️')}});
